...
int main (int argc, char * argv[])
{
  ...
  {   if(pass < 3)
    ...
    if(TFflag){
      ...
      legends.push_back(new TLegend(0.75, 0.45, 0.97, 0.92, first_key->GetTitle()));
      legends.back()->SetFillStyle(0);
      legends.back()->SetBorderSize(0);
    } 
  ...
  if(TFflag){
    ...
    {
      std::cout << "file: " << argv[3 * i + 3] << std::endl;
      // hist file
      ...
      {   
        ...
        if (atoi(argv[3 * i + 5]))
        {  
          std::cout << "add to stack: " << argv[3 * i + 4] << std::endl;
          hists.back()->SetFillColor(color[i]);
          hists.back()->SetFillStyle(1001);
          // add it to the stacked histograms
          error.at(j)->SetMarkerSize(0);
          error.at(j)->SetFillColor(kBlue);
          error.at(j)->SetFillStyle(3365);
        ...
        std::cout << "add to legend: " << argv[3 * i + 4] << std::endl;
    ...
    for (int i = 0; i < file_num; i++)
    {
      ...
      {   
        ...
        {
          std::cout << "add to signal: " << argv[3 * i + 4] << std::endl;
  ...
  if(TFflag){
    ...
    for (int k = 0; k < stacked_hists.size(); k++)
    {
      ...
      //if (not_stacked_hists.at(k)->GetNhists() == 0) 
      //{
